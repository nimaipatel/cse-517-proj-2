...
main(void)
{
    ...
    for (size_t i = 0; i < NUM_TRACES; i += 1) {
        ...
        CSV_Write(f, basename(traces[i]), malloc.mean, malloc.margin_of_error, realloc.mean, realloc.margin_of_error, free.mean,
                  free.margin_of_error, total.mean, total.margin_of_error, result.util);
